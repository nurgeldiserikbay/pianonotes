...
Разделение работы: **визуал рисует GPT, подключение делаю я.** Чтобы вам не
приходилось ждать меня после каждой картинки, все рисованные элементы в
приложении сделаны *слотами*: путь, который подхватывается сам.

**Как это работает:** кладёте файл по указанному пути → перезагружаете страницу
→ картинка на месте. Файла нет или он битый → рисуется запасной вариант кодом,
ничего не ломается и ошибок в консоли не появляется. Можно присылать по одному
файлу, полусобранный комплект работает.

Механика слота — [`src/ui/useDropInArt.ts`](src/ui/useDropInArt.ts).

## Таблица слотов

| Файл | Размер | Где видно | Если файла нет |
| --- | --- | --- | --- |
| `public/img/mascot/cat-idle.png` и ещё 4 позы | 1024×1024, прозрачный | подсказка в меню, экран результата | один из трёх готовых котов `/img/stitch/*` |
| `public/img/logo-piano-notes.png` | 1600×500, прозрачный | шапка меню | надпись «Piano Notes» текстом |
| `public/img/moods/bright.png` и ещё 5 настроений | 1280×480, без прозрачности | «герой» в меню, карточки глав | векторная сцена, рисуется кодом |
| `resources/icon.png` | 1024×1024, без прозрачности | иконка приложения | текущая иконка |

Иконка — единственный пункт не «на лету»: после замены файла нужна пересборка
(`npx capacitor-assets generate`), это делаю я.

Общие требования ко всем PNG: **прозрачный фон** (кроме обложек и иконки), без
подписей и текста внутри картинки, без рамок и теней «под объектом» (тени рисует
приложение), сюжет по центру с полем 6–8% по краям.
...
Обложка обрезается по краям (`object-fit: cover`) и снизу на неё ложится
затемнение с названием мелодии — сюжет держите в верхних двух третях, низ
оставляйте спокойным.
...
## 5. Праздничные спрайты (необязательно, слота пока нет)

Эти два файла ещё никуда не подключены — сейчас победный эффект рисуется кодом.
Если пришлёте, сделаю под них слот и заменю.
